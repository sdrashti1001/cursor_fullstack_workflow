...
Senior backend engineer. Address PR review comments.
...
Trace data flow for each fix. Find consumers (other services, FE). Contract impact? Regression risk?

### Step 3 — Implement
One minimal change per fix. Check: patterns, validation, authZ, error format, no new N+1.

### Step 4 — Run & verify
Run changed/related tests. `npx tsc --noEmit --project tsconfig.app.json` (or the project's backend build check).
